    ...
    public void keyPress(long pWindowPointer, int pKey, int pScanCode, int pAction, int pModifiers) {
        if (pWindowPointer == this.minecraft.getWindow().getWindow()) {
            ...
            if ((!(this.minecraft.screen instanceof KeyBindsScreen) || ((KeyBindsScreen)screen).lastKeySelection <= Util.getMillis() - 20L)) {
                if (pAction == 1) {
                    if (mc.screen == null) {
                        EventManager.call(new EventInputKey(pKey));
                    }
                }
                ...
                Screen.wrapScreenError(() -> {
                    if (pAction == 1 || pAction == 2) {
                        ...
                        if (Reflector.ForgeHooksClient_onScreenKeyPressed.exists()) {
                            Reflector.call(Reflector.ForgeHooksClient_onScreenKeyPressed, aboolean, screen, pKey, pScanCode, pModifiers);
                        } else {
                            aboolean[0] = screen.keyPressed(pKey, pScanCode, pModifiers);
                        }
                    } else if (pAction == 0) {
                        if (Reflector.ForgeHooksClient_onScreenKeyReleased.exists()) {
                            Reflector.call(Reflector.ForgeHooksClient_onScreenKeyReleased, aboolean, screen, pKey, pScanCode, pModifiers);
                        } else {
            ...
            if (screen != null && this.minecraft.getOverlay() == null) {
                if (Character.charCount(pCodePoint) == 1) {
                    ...
                    for (char c0 : Character.toChars(pCodePoint)) {
                        Screen.wrapScreenError(() -> {
                            if (Reflector.ForgeHooksClient_onScreenCharTyped.exists()) {
                                Reflector.call(Reflector.ForgeHooksClient_onScreenCharTyped, screen, c0, pModifiers);
                            } else {
                                screen.charTyped(c0, pModifiers);
                            }
                        }, "charTyped event handler", screen.getClass().getCanonicalName());
                    }
                }
