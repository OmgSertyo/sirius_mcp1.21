            ...
            if (!pPlayer.isScoping()) {
                ...
                if (pStack.isEmpty()) {
                    ...
                    if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) {
                        ...
                        float f10 = (float)pStack.getUseDuration(pPlayer) - ((float)pPlayer.getUseItemRemainingTicks() - pPartialTicks + 1.0F);
                        float f14 = f10 / (float)CrossbowItem.getChargeDuration(pStack, pPlayer);
                        if (f14 > 1.0F) {
                            f14 = 1.0F;
                        }
                        ...
                        float f13 = 0.2F * Mth.sin(Mth.sqrt(pSwingProgress) * (float) (Math.PI * 2));
                        ...
                        if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) {
                            ...
                            switch (pStack.getUseAnimation()) {
                                ...
                                case SPEAR:
                                    ...
                                    pPoseStack.mulPose(Axis.YP.rotationDegrees((float)k * 35.3F));
                                    pPoseStack.mulPose(Axis.ZP.rotationDegrees((float)k * -9.785F));
                                    float f11 = (float)pStack.getUseDuration(pPlayer) - ((float)pPlayer.getUseItemRemainingTicks() - pPartialTicks + 1.0F);
                                    float f2 = f11 / 10.0F;
                                    if (f2 > 1.0F) {
                                        f2 = 1.0F;
                                    }
                            ...
                            float f = 0.2F * Mth.sin(Mth.sqrt(pSwingProgress) * (float) (Math.PI * 2));
                            float f1 = -0.2F * Mth.sin(pSwingProgress * (float) Math.PI);
                            int j = flag1 ? 1 : -1;
