...
public class ShaderSirius {
    ...
    public ShaderSirius(String fragmentSource, boolean isFile) {
        int shaderProgram = GL20.glCreateProgram();
        int fragmentShader = GL20.glCreateShader(35632);

        try {
            GL20.glShaderSource(fragmentShader, isFile ? this.readInputStream(Minecraft.getInstance().getResourceManager().getResource(new ResourceLocation(fragmentSource)).get().open()) : fragmentSource);
        ...
        GL20.glShaderSource(vertexShader, "#version 150\nvoid main() {\n gl_TexCoord[0] = gl_MultiTexCoord0;\n gl_Position = gl_ModelViewProjectionMatrix * gl_Vertex;\n}");
    ...
    public ShaderSirius(String fragmentSource, String vertexSource, boolean isFile) {
        int shaderProgram = GL20.glCreateProgram();
        int fragmentShader = GL20.glCreateShader(35632);
        ...
        try {
            GL20.glShaderSource(vertexShader, isFile ? this.readInputStream(Minecraft.getInstance().getResourceManager().getResource(new ResourceLocation(vertexSource)).get().open()) : vertexSource);
